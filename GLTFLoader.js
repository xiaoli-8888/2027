...
function toTrianglesDrawMode( geometry, drawMode ) {
  ...
  if ( drawMode === THREE.TriangleStripDrawMode ) {
    ...
    geometry.setIndex(ni); geometry.groups = [];
  } else if ( drawMode === THREE.TriangleFanDrawMode ) {
    var ni = [];
    if ( indices !== null ) {
      for ( var i = 1; i < indices.length - 1; i ++ ) ni.push(indices[0], indices[i], indices[i+1]);
    } else {
      for ( var i = 1; i < vertices.length/3 - 1; i ++ ) ni.push(0, i, i+1);
    }
    geometry.setIndex(ni); geometry.groups = [];
  }
  return geometry;
